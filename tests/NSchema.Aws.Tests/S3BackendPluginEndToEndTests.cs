...
/// <summary>
/// End-to-end proof that the <see cref="S3BackendPlugin"/> manifest wires a working state store: it builds a host
...
/// </summary>
[Collection("minio")]
public sealed class S3BackendPluginEndToEndTests(MinioFixture fixture)
{
    [Fact]
    public async Task Configure_ThroughThePlugin_RoundTripsStateAgainstS3()
    {
        ...
        var configured = new S3BackendPlugin().Configure(builder, new ConfigBlock("backend", "s3", new Dictionary<string, ConfigValue>
        {
            ["bucket"] = ConfigValue.OfString(fixture.BucketName),
            ["key"] = ConfigValue.OfString(key),
        }));
        configured.Succeeded.ShouldBeTrue();

        using var app = builder.Build();
        var store = app.Services.GetRequiredService<ISchemaStateStore>();
